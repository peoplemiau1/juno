  ...
  def tokenize
    ...
    while cursor < @code.length
      ...
      when /\A(\/\/|#).*$/
        cursor += $&.length
        @column += $&.length
      when /\A(struct|union|fn|func|def|if|elif|else|return|while|loop|break|continue|let|for|import|use|packed|extern|from|pub|match|todo|panic|as|true|false|mut|type|enum|real|int|string|bool)\b/
