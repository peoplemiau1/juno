...
module BuiltinHeap
  ...
  def gen_malloc(node)
    ...
    @emitter.emit([0x41, 0x53]) # push r11 (preserve size)
    @emitter.emit([0xb8, 0x09, 0x00, 0x00, 0x00])  # eax = 9
    @emitter.emit([0x0f, 0x05])
    @emitter.emit([0x41, 0x5b]) # pop r11 (restore size)
  ...
  def gen_realloc(node)
    ...
    @emitter.emit([0xeb, 0x30]) # jmp end (placeholder)
    realloc_err_jmp = @emitter.current_pos - 1

    @emitter.emit([0x48, 0x89, 0x18, 0x48, 0x83, 0xc0, 0x08]) # store header, rax = new_ptr
    ...
    # 3. Free old memory
    @emitter.emit([0x50]) # push rax (preserve new_ptr)
    @emitter.emit([0x4c, 0x89, 0xff]) # mov rdi, r15
    @emitter.emit([0x48, 0x83, 0xef, 0x08]) # sub rdi, 8
    @emitter.emit([0x48, 0x8b, 0x37]) # mov rsi, [rdi]
    @emitter.emit([0xb8, 0x0b, 0x00, 0x00, 0x00]) # munmap
    @emitter.emit([0x0f, 0x05])
    @emitter.emit([0x58]) # pop rax (restore new_ptr)

    # 4. Done
    target = @emitter.current_pos
    @emitter.bytes[jz_pos + 1] = (target - (jz_pos + 2)) & 0xFF
    @emitter.bytes[realloc_err_jmp] = (target - (realloc_err_jmp + 1)) & 0xFF
  ...
  def gen_free(node)
    return unless @target_os == :linux
    return if !node[:args] || node[:args].empty?
