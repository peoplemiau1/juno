...
module BuiltinHTTPS
  ...
  def gen_curl_get(node)
    ...
    @emitter.emit([0,0,0,0, 0xba, 0xff, 0x1f, 0, 0, 0xb8, 0,0,0,0, 0x0f, 0x05, 0x8b, 0x3c, 0x24, 0xb8, 0x03, 0,0,0, 0x0f, 0x05])
    ...
    # Close read end
    @emitter.emit([0x8b, 0x3c, 0x24, 0xb8, 0x03, 0,0,0, 0x0f, 0x05])
  ...
  def gen_https_request(node)
    ...
    @emitter.emit([0x8b, 0x7c, 0x24, 0x04, 0xb8, 0x03, 0,0,0, 0x0f, 0x05, 0x8b, 0x3c, 0x24, 0x48, 0x8d, 0x35])
    @linker.add_data_patch(@emitter.current_pos, "https_buf")
    @emitter.emit([0,0,0,0, 0xba, 0xff, 0x1f, 0, 0, 0xb8, 0,0,0,0, 0x0f, 0x05, 0x8b, 0x3c, 0x24, 0xb8, 0x03, 0,0,0, 0x0f, 0x05, 0x48, 0xc7, 0xc7, 0xff, 0xff, 0xff, 0xff, 0x48, 0x8d, 0x74, 0x24, 0x08, 0x31, 0xd2, 0x4d, 0x31, 0xd2, 0xb8, 0x3d, 0,0,0, 0x0f, 0x05])
    @emitter.emit([0x48, 0x8d, 0x05])
    @linker.add_data_patch(@emitter.current_pos, "https_buf")
    @emitter.emit([0,0,0,0, 0x48, 0x83, 0xc4, 0x10])
    parent_end = @emitter.current_pos
