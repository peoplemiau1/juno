module BuiltinHTTPS
  ...
  def gen_curl_get(node)
    ...
    setup_https_data
    @emitter.emit([0x41, 0x54, 0x41, 0x55, 0x41, 0x56, 0x41, 0x57])
    ...
    @emitter.emit([0x41, 0x5f, 0x41, 0x5e, 0x41, 0x5d, 0x41, 0x5c])
  end
  ...
  def gen_https_request(node)
    ...
    @emitter.emit([0,0,0,0, 0x48, 0x89, 0x44, 0x24, 0x10, 0x4c, 0x89, 0x64, 0x24, 0x18, 0x48, 0x8d, 0x05])
    @linker.add_data_patch(@emitter.current_pos, "curl_H")
    @emitter.emit([0,0,0,0, 0x48, 0x89, 0x44, 0x24, 0x20, 0x4c, 0x89, 0x74, 0x24, 0x28, 0x48, 0x8d, 0x05])
    @linker.add_data_patch(@emitter.current_pos, "curl_d")
    @emitter.emit([0,0,0,0, 0x48, 0x89, 0x44, 0x24, 0x30, 0x4c, 0x89, 0x7c, 0x24, 0x38, 0x4c, 0x89, 0x6c, 0x24, 0x40, 0x48, 0xc7, 0x44, 0x24, 0x48, 0,0,0,0])
    @emitter.emit([0x48, 0x8d, 0x3d])
    @linker.add_data_patch(@emitter.current_pos, "curl_path")
    @emitter.emit([0,0,0,0, 0x48, 0x89, 0xe6, 0x48, 0x31, 0xd2, 0xb8, 0x3b, 0,0,0, 0x0f, 0x05])
