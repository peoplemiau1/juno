module LLVMStatementGenerator
  ...
  def gen_llvm_insertC(node)
    ...
    # Маппинг сишных регистров в формат ограничений (constraints) LLVM
    clobber_list = (node[:clobbers] || []).map do |c|
      if c.to_s.downcase == 'flags'
        @arch == :aarch64 ? "~{cc}" : "~{flags},~{fpsr}"
      else
        "~{#{c.to_s.downcase}}"
      end
    end
    constraints = clobber_list.join(",")

    @output << "  call void asm sideeffect \"#{asm_instruction}\", \"#{constraints}\"()\n"
  end
end
